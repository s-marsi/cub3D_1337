...
/*   Updated: 2025/01/29 23:01:36 by smarsi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static double	get_y_render(t_init *vars, double n, int ray)
{
	...
	else if (vars->rays[ray].ray_side == 5)
...
void	render_wall(t_init *vars, int ray)
{
	...
	while (i < vars->rays[ray].b_pix)
	{
		...
		int map_g_i_x = floor(vars->rays[ray].ray_x / vars->tile_size);
		int map_g_i_y = floor(vars->rays[ray].ray_y / vars->tile_size);
		char map_cell = vars->map[map_g_i_y][map_g_i_x];
		if (map_cell == 'D')
		{
			vars->rays[ray].ray_side = 5;
